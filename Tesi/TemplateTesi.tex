...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Nearest Insertion, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
    \State $path = [0, 0]$
    \State $n$ = numero di città
    \State $distances$ = matrice di adiacenza
    ...
    \For{$i \in \{0, 1, 2, ..., n - 1\}$}
      \For{$j \in \{0, 1, 2, ..., i - 1\}$}
        \If{distances[i][j] < minDist}
          ...
          \State \texttt{minDist = distances[i][j]}
        \EndIf
      \EndFor
    \EndFor
    \State Sia $h$ lo heap
    \For{$i \not \in path$}
      \State $left \gets distances[path[0], i]$
      \State $right \gets distances[path[1], i]$
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Nearest Insertion, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
    \State ...
    \While{\texttt{len(path) < n}}
      \State Ottieni dallo heap il record $r$ con costo minore
      \State Ottieni il nodo $to\_insert$ dal record $r$
      \State Sia $best\_increase \gets \infty$
      \State Sia $best\_position \gets -1$
      \For{$i \in \{0, 1, 2, ..., len(path)\}$}
        \State $next\_i \gets (i + 1) \mod \text{len(path)}$
        \State $increase \gets distances[path[i]][to\_insert] + distances[to\_insert, path[next\_i]] - distances[path[i], path[next\_i]]$
        \If{$increase < best\_increase$}
          \State $best\_increase \gets increase$
          \State $best\_position \gets i + 1$
        \EndIf
      \EndFor
      \State \text{inserisci to\_insert nel path in posizione best\_position}
      \For{(cost, node) nello heap h}
        \If{$node \not \in path \land distances[to\_insert][node] < cost$}
          \State $(cost, node) \gets (distances[to\_insert][node], node)$
        \EndIf
      \EndFor
      \State heapify(h)
    \EndWhile
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Cheapest Insertion Versione 1, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Cheapest Insertion Versione 2, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
    \State $n \gets$ numero di città
    \State $adj \gets$ matrice di adiacenza dell'istanza
    \State $path \gets [0, 0]$
  ...
  \begin{algorithmic}[1]
    ...
    \While{$len(path) < n$}
      ...
      \For{$h_i \in h$}
        ...
        \EndIf
        \If{$node \not \in in\_path and left\_cost < cost$}
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Cheapest Insertion Versione 2 Approssimata, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
    \State ...
    \While{$len(path) < n$}
      \State \dots
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Cheapest Insertion Versione 3, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
    \State $n \gets$ numero di città
    \State $adj \gets$ matrice di adiacenza dell'istanza
    \State $path \gets [0, 0]$
...
In conclusione si procede con il ripristino della struttura dello heap piccolo (riga 25). \newline Importante notare l'istruzione alla riga 28: concluso l'aggiornamento di ogni heap, si procede limitando la dimensione degli heap piccoli a \lstinline!m!: questo parametro è necessario perché con \lstinline|m = n| gli heap piccoli \lstinline|hp| avrebbero nel caso peggiore una dimensione pari a \lstinline|n| e quindi si otterrebbe una complessità temporale pari a $O(n^2 * log(n))$: sono presenti due cicli che iterano n volte (riga 1 e 3) e poi l'istruzione heapify che ha complessità $log(m)$ dove $m$ indica la lunghezza dello heap. Per questo motivo è necessario tenere il parametro m il più basso possibile in modo da limitare la complessità temporale, ma la complessità spaziale: infatti sarebbe necessario tenere in memoria $O(n^2)$ record, mentre limitando il parametro si avrebbe complessità spaziale pari a $O(nm)$ per quanto riguarda il mantenimento dei record. \newline
Questa implementazione riduce la complessità temporale delle versioni precedenti da $O(n^3)$ a $O(n^2 * log(n))$ e limitando la dimensione degli heap, si ottiene complessità $O(n^2 * log(m))$ con $m < n$ pur garantendo che, ad ogni iterazione, si selezioni il nodo che minimizza il costo di inserimento e si selezioni il punto di inserimento ottimo (nella versione approssimata vista in precedenza questa garanzia non c'era). \newline
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Farthest Insertion, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
    \State $n \gets$ numero di città
    \State $dist \gets$ matrice di adiacenza dell'istanza
    \State $path \gets [0, 0]$
    ...
    \State Sia $h$ lo heap
    \For{$i \not \in path$}
      \State $left \gets dist[path[0], i]$
      \State $right \gets dist[path[1], i]$
      \State Aggiungi $(-min\{left, right\}, i)$ allo heap $h$
    \EndFor
    \State heapify(h)
    \State ...
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Farthest Insertion, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
      ...
      \For{$i \in \{0, 1, ..., len(path) - 1\}$}
        ...
        \State $increase = dist[node, to\_ins] + dist[to\_ins, next] - dist[node, next]$
        \If{$increase < best\_increase$}
          \State $best\_increase \gets increase$
          \State $best\_position \gets i + 1$
        \EndIf
      ...
      \For{$h_i \in h$}
        ...
        \If{$node \not \in in\_path \land dist[to\_ins, node] < cost$}
          \State $h_i \gets (-dist[to\_ins, node], node)$
        \EndIf
      \EndFor
      \State heapify(h)
    \EndWhile
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Furthest Insertion Versione 1, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
    \State $n \gets$ numero di città
    \State $adj \gets$ matrice di adiacenza dell'istanza
    \State $path \gets [0, 0]$
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Furthest Insertion Versione 2, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
    \State $n \gets$ numero di città
    \State $adj \gets$ matrice di adiacenza dell'istanza
    \State $path \gets [0, 0]$
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Furthest Insertion Versione 3, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
    \State $n \gets$ numero di città
    \State $adj \gets$ matrice di adiacenza dell'istanza
    \State $path \gets [0, 0]$
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Random Insertion Versione 1, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Random Insertion Versione 2, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Nearest Insertion Random Start, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
    \State Sia $n$ il numero di città presenti nell'istanza
    \State Sia $dist$ la matrice di adiacenza
...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=0.5pt, title=Nearest Neighbor Random Start, coltitle=black, fonttitle=\bfseries, colbacktitle=white]
  \begin{algorithmic}[1]
    \State Sia $n$ il numero di città presenti nell'istanza
    \State Sia $dist$ la matrice di adiacenza
