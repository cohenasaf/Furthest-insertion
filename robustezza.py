...
for l in TSP_LIB[:50]:
    ...
    for i in range(20):
        NN2.append(testEuristic("nearestNeighbor", t.nearestNeighborRandomStart))
        NI2.append(testEuristic("nearestInsertion", t.nearestInsertionRandomStart))
        CI2.append(testEuristicO("cheapestInsertionOttimizzato", t.cheapestInsertionOttimizzatoRandomStart, dC[l]))
        FaI2.append(testEuristic("farthestInsertion", t.farthestInsertionRandomStart))
        FuI2.append(testEuristic("furthestInsertion", t.furthestInsertionRandomStart))
        # media qualità, var qualità, media tempi, var tempi
    NN.append((np.mean([x[0] for x in NN2]), np.var([x[0] for x in NN2]), np.mean([x[1] for x in NN2]), np.var([x[1] for x in NN2])))
    NI.append((np.mean([x[0] for x in NI2]), np.var([x[0] for x in NI2]), np.mean([x[1] for x in NI2]), np.var([x[1] for x in NI2])))
    CI.append((np.mean([x[0] for x in CI2]), np.var([x[0] for x in CI2]), np.mean([x[1] for x in CI2]), np.var([x[1] for x in CI2])))
    FaI.append((np.mean([x[0] for x in FaI2]), np.var([x[0] for x in FaI2]), np.mean([x[1] for x in FaI2]), np.var([x[1] for x in FaI2])))
    FuI.append((np.mean([x[0] for x in FuI2]), np.var([x[0] for x in FuI2]), np.mean([x[1] for x in FuI2]), np.var([x[1] for x in FuI2])))
